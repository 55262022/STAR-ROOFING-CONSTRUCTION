...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($image_path) {
        $stmt = $conn->prepare("UPDATE employees 
            SET first_name = ?, last_name = ?, email = ?, phone = ?, position = ?, department = ?, 
                hire_date = ?, salary = ?, status = ?, image_path = ? 
            WHERE employee_id = ?");
        $stmt->bind_param("sssssssdssi", $first_name, $last_name, $email, $phone, $position, $department, 
        ...
        $stmt = $conn->prepare("UPDATE employees 
            ...
            WHERE employee_id = ?");
        $stmt->bind_param("sssssssdsi", $first_name, $last_name, $email, $phone, $position, $department, 
                          $hire_date, $salary, $status, $employee_id);
    }

    if ($stmt->execute()) {
        header("Location: ../admin/employees.php?success=Employee updated successfully");
    } else {
        header("Location: ../admin/employees.php?error=Error updating employee: " . $conn->error);
    }
